"""Fault-isolated Excel import: no individual row can abort a batch."""
import json
import logging
...
def _review(db: Session, batch: ImportBatch, row_number: int, raw: list[Any], exc: Exception, action: str, values: dict[str, str] | None = None) -> None:
    values = values or {}
...
def import_workbook(db: Session, contents: bytes, filename: str, imported_by_id: int | None) -> ImportBatch:
    ...
    try:
        ...
        for row_number, raw_tuple in enumerate(rows, start=2):
            raw = list(raw_tuple); batch.total_rows += 1
            values: dict[str, str] = {}
            try:
                values = {field: _text(raw[index]) if index < len(raw) else "" for field, index in mapping.items()}
                task_number = values.get("task_number", "")
                if not task_number:

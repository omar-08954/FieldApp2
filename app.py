            ...
            with timed_spinner("💾 جاري حفظ المهمة..."):
                ...
                if assigned_match:
                    ...
                    notify_admins(
                        "task_status",
                        "تنفيذ مهمة مسندة",
                        f"قام {st.session_state.fullname} بتنفيذ المهمة رقم {task_number} (الحالة: {task_status}).",
                    )
                    notify_user(
                        st.session_state.username,
                        "task_status",
                        "تم تنفيذ المهمة",
                        f"تم تسجيل تنفيذك للمهمة رقم {task_number} بنجاح.",
                    )
                else:
                    ...
                    if not exists:
                        ...
                        notify_admins(
                            "task_add",
                            "مهمة جديدة",
                            f"أضاف {st.session_state.fullname} مهمة جديدة برقم {task_number}.",
                        )
                        notify_user(
                            st.session_state.username,
                            "task_add",
                            "تم تسجيل المهمة",
                            f"تم تسجيل مهمتك رقم {task_number} بنجاح.",
                        )
            if exists:
            ...
            with timed_spinner("💾 جاري حفظ المهمة..."):
                ...
                if not exists:
                    ...
                    notify_technician_by_name(
                        ...
                        f"أسند إليك {st.session_state.fullname} المهمة رقم {task_number}.",
                        toast_icon="📌",
                    )
                    notify_admins(
                        "task_assign",
                        "إسناد مهمة",
                        f"أسند {st.session_state.fullname} المهمة رقم {task_number} إلى {technician}.",
                    )
            if exists:
                st.error("❌ لا يمكن إضافة نفس رقم المهمة مرتين.")
            else:
    ...
    with tab_data:
                ...
                with timed_spinner("جاري حفظ التعديلات..."):
                    ...
                    if not duplicate:
                        ...
                        notify_technician_by_name(
                            ...
                            "task_update",
                            "تعديل مهمة",
                            f"قام {st.session_state.fullname} بتعديل المهمة رقم {new_number}.",
                        )
                        notify_admins(
                            "task_update",
                            "تعديل مهمة",
                            f"قام {st.session_state.fullname} بتعديل المهمة رقم {new_number} (الفني: {task.get('technician')}).",
                        )
                if duplicate:
        ...
        if confirm_delete_button("delete_tasks", selected_ids, "🗑 حذف المهام المحددة", "مهمة محددة"):
            with timed_spinner("جاري حذف المهمة..."):
                ...
                affected = delete_df[delete_df["id"].astype(int).isin(selected_ids_int)]
                deleted_info = affected[["technician", "task_number"]].to_dict("records")
            ...
            for item in deleted_info:
                notify_technician_by_name(
                    ...
                    "task_delete",
                    "حذف مهمة",
                    f"قام {st.session_state.fullname} بحذف المهمة رقم {item.get('task_number')}.",
                )
            if deleted_info:
                notify_admins(
                    "task_delete",
                    "حذف مهام",
                    f"قام {st.session_state.fullname} بحذف {len(deleted_info)} مهمة.",
                )
        ...
        if add_submitted:
            ...
            if not fullname.strip() or not username.strip() or not password.strip() or not confirm_password.strip():
                ...
                try:
                    ...
                    event_type = "technician_add" if ROLE_VALUES[role_label] == "technician" else "user_add"
                    notify_admins(
                        event_type,
                        "مستخدم جديد",
                        f"أضاف {st.session_state.fullname} مستخدماً جديداً: {fullname} ({username}).",
                    )
                    notify_user(
                        username,
                        event_type,
                        "تم إنشاء حسابك",
                        f"تم إنشاء حسابك في النظام بواسطة {st.session_state.fullname}.",
                    )
                ...
                if not fullname.strip():
                    ...
                    if password.strip():
                        notify_user(
                            ...
                            f"قام {st.session_state.fullname} بإعادة تعيين كلمة مرورك.",
                        )
                        notify_admins(
                            "password_reset",
                            "إعادة تعيين كلمة مرور",
                            f"قام {st.session_state.fullname} بإعادة تعيين كلمة مرور المستخدم {selected_user['username']}.",
                        )
                    st.success("✅ تم حفظ التعديلات بنجاح")
                    st.rerun()
            ...
            if not is_strong:
                ...
                if valid:
                    ...
                    notify_admins(
                        "password_reset",
                        "تغيير كلمة مرور",
                        f"قام {st.session_state.fullname} بتغيير كلمة مروره بنفسه.",
                    )

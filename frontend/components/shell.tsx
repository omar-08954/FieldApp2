...
const links = [
  ["لوحة التحكم", "/dashboard", LayoutDashboard], ["لوحة المدير", "/admin", ShieldCheck],
  ["المهام", "/tasks", ClipboardList], ["صفحة الفني", "/technician", Wrench],
  ["التقارير", "/reports", FileSpreadsheet], ["المستودع", "/inventory", Package],
  ["إدارة المستخدمين", "/users", Users], ["مركز المطور", "/developer", Code2],
  ["مراجعة الاستيراد", "/imports", UserCog], ["الإعدادات", "/settings", Settings],
] as const;
...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  const { data: user } = useQuery({ queryKey: ["current-user"], queryFn: () => api<CurrentUser>("/auth/me"), enabled: Boolean(token()), retry: false });

...
export function ImportReviewPanel() {
  ...
  const [message, setMessage] = useState(""); const [sourceName, setSourceName] = useState(""); const [targetName, setTargetName] = useState("");
  const refresh = () => client.invalidateQueries({ queryKey: ["import-reviews"] });
  async function bulkCorrect(event: React.FormEvent) { event.preventDefault(); if (!sourceName || !targetName) return; try { const result = await api<{ matched: number; inserted: number; failed: number }>("/import-reviews/bulk-technician", { method: "POST", body: JSON.stringify({ source_name: sourceName, target_name: targetName, reinsert: true }) }); setMessage(`تمت مطابقة ${result.matched} صف، وإعادة إدراج ${result.inserted} صف${result.failed ? `، وبقي ${result.failed} صف للمراجعة` : ""}.`); refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "تعذر تنفيذ التصحيح الجماعي."); } }
